...
class HomeController extends Controller
{
    ...
    public function profile(Request $request)
    {
        ...
        return view('front.user.profile',['users'=>$users,'paymentdetails'=>$paymentdetails,'doctor_days_checked'=>$doctor_days_checked,'doctor_days'=>$doctor_days,'doctor_daysendtime'=>$doctor_daysendtime,'language' => $language,'doctor_language' => $doctor_language]);
    }
        ...
        if ($validator->fails()) {
            ...
            if(isset($medicines))
            {
                ...
                foreach ($medicines as $key => $e) {
                    ...
                    if($e != null)
                    {
                        $visit_prescription_data = new Visit_Prescription;
                        $visit_prescription_data->emr_id=isset($emr_lastId)?$emr_lastId:'';
                       // $visit_prescription->medicine_id = $e ? $e:"";
                        $medicine_name=Medicine::select('name')->where('id',$e)->first();
                        //$visit_prescription->medicine_name = $medicine_name->name;
                        //$visit_prescription->dose = $dose[$key] ? $dose[$key]: "";
                        //$visit_prescription->unit = $dosetype[$key] ? $dosetype[$key]:"";
                        //$visit_prescription->frequency = $frequency1[$key] ? $frequency1[$key]:"0";
                        //$visit_prescription->frequency2 = $frequency2[$key] ? $frequency2[$key]: "0";
                        //$visit_prescription->frequency3 = $frequency3[$key] ? $frequency3[$key]:"0";
                        ...
                        $patient_name=User::where('id',request('doctor_id'))->first();
                        ...
                        $pdf = PDF::loadView('admin.patient.visit_pdf',compact('visit_prescription_data','clinic','doctor_name','patient_name'))->save($path);
