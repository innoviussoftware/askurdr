...
use App\Helpers\Notification\Paytab;
    ...
    */
    ...
    public function me(Request $request)
    {
        ...
        if ($user) {
            ...
            if($userrole['0']->name == "doctor")
            {
                ...
                $DoctorAvailability = DoctorAvailability::where('patient_id',$user->id)->first();
                $DoctorAvailability->patient_id = (string) $DoctorAvailability->patient_id;
                $DoctorAvailability->status = (string) $DoctorAvailability->status;
                $user['availability'] = $DoctorAvailability;
                $user['speak_language'] = '';
            }
            ...
            $user->is_insurance =  (string) $user->is_insurance;
            $user->payment_type =  (string) $user->payment_type;
            $user->wallet_money =  (string) $user->wallet_money;
            $user->sinch_ticket =  (string) $user->sinch_ticket;
    ...
    public function specialitydoctoreList(Request $request)
    {
        ...
        if(request('language') == 'ar'){
            if(request('consultant') == 2){
                $DoctorSpeciality=DoctorSpeciality::select('doctor_speciality.*','users.ar_askid as ask_doctor_id','users.ar_first_name as first_name','users.national_id','users.fees as fees','users.last_name','users.mobile','users.profile_pic','users.gender','users.discount','doctor_availability.status','doctor_education.ar_course','doctor_experience.year','users.ar_doctorcode as group_code',
                DB::raw('users.fees * users.discount /100  as discountprice')
                )
                ...
                foreach ($DoctorSpeciality as $key => $value) {
                    if($value->discount > 0)
                    {
                        $fees=$value->fees*$value->discount/100;
                        $value['orignalFees'] = strval($fees);
                    }
                    else
                    {
                        $fees=$value->fees;
                    ...
                    if($doctorid){
                        ...
                        $value['call_type'] = isset($followupdate->call_type)?$followupdate->call_type:'';
                    }
                }
            }else{
                $DoctorSpeciality=DoctorSpeciality::select('doctor_speciality.*','users.ar_askid as ask_doctor_id','users.ar_first_name as first_name','users.national_id','users.fees as fees','users.last_name','users.mobile','users.profile_pic','users.gender','users.discount','doctor_availability.status','doctor_education.ar_course','doctor_experience.year','users.ar_doctorcode as group_code',
            ...
            if(request('consultant') == 2){
                $DoctorSpeciality=DoctorSpeciality::select('doctor_speciality.*','users.ask_id as ask_doctor_id','users.first_name','users.national_id','users.fees','users.last_name','users.mobile','users.profile_pic','users.gender','users.fees','users.discount','doctor_availability.status','doctor_education.course','doctor_experience.year','users.doctor_code as group_code',
                DB::raw('users.fees * users.discount /100  as discountprice')
                )
                ->join('users', 'doctor_speciality.user_id', '=', 'users.id')
    ...
    public function specialitydoctorelistwithfilter(Request $request)
    {
        ...
        if($request->gender == 'all'){
            if(request('language') == 'ar'){
            $DoctorSpeciality=DoctorSpeciality::select('doctor_speciality.*','users.ar_askid as ask_doctor_id','users.ar_first_name as first_name','users.national_id','users.last_name','users.mobile','users.profile_pic','users.gender','users.fees as fees','users.discount','doctor_availability.status','doctor_education.ar_course','doctor_experience.year','users.ar_doctorcode as group_code',
            ...
            if(request('language') == 'ar'){
            $DoctorSpeciality=DoctorSpeciality::select('doctor_speciality.*','users.ar_askid as ask_doctor_id','users.ar_first_name as first_name','users.national_id','users.last_name','users.mobile','users.profile_pic','users.gender','users.fees as fees','users.discount','doctor_availability.status','doctor_education.ar_course','doctor_experience.year','users.ar_doctorcode as group_code',DB::raw('users.fees * users.discount /100  as discountprice')
            )->join('users', 'doctor_speciality.user_id', '=', 'users.id')
            ->leftjoin('doctor_availability', 'doctor_speciality.user_id', '=', 'doctor_availability.doctor_id')
            ->leftjoin('doctor_education', 'doctor_speciality.user_id', '=', 'doctor_education.user_id')
            ->leftjoin('doctor_experience', 'doctor_speciality.user_id', '=', 'doctor_experience.user_id')
    ...
    public function addwalletMoney(Request $request)
    {
        ...
        $money=User::where('id',$user_id)->first();

        $totalprice=$money->wallet_money+$amount;

        $followupdate=User::where('id',$user_id)->update(['wallet_money'=>$totalprice]);

        $fname=User::where('id',$user_id)->first();

        $payment_history = new Payment_history;
        $payment_history->user_id = auth()->user()->id;
        $payment_history->user_id2 = '-';
        $payment_history->price = $amount;  // admin id
        $payment_history->message = 'Payment added via card '.$amount.'SAR';
        $payment_history->status = 1;
        $payment_history->save();

        
          return response()->json(['success' => true,'totalbalance'=>$totalprice], 200);
    }
    ...
    public function chatType(Request $request)
    {
        ...
        $fname=User::where('id',$patient_id)->first();

        $emr_number=$fname->emr_number;
        ...
        if($followupdate)
        {
            $newarray=array(
                'emr_no'=>$emr_number,
                'patientid'=>$followupdate->patient_id,
                'doctorid'=>$followupdate->doctor_id,
                'followupdate'=>$followupdate->followup_date,
                'followupcalltype'=>$followupdate->call_type
            );
        }

        return response()->json(['success' => true,'EmrDetails'=>$newarray], 200);
    }

    public function addpaymentPaytab(Request $request)
    {
        ...
        $array=array(
            ...
            'merchant_email' => "shahidpatel.innovius@gmail.com",
            'secret_key' => "OuowHCKfQbN2xnxRiZ68O837wcReUeOJIcfqO0GFbtHIReA9dGi2Plamy2AshobZAqqpq64CAMlr1WBfDThsfsONupfLNrLoScOj",
           ...
           // //Product Information
            "products_per_title" => "Call",   //Product title of the product. If multiple products then add “||” separator
           ...
           //  //Website Information
           "site_url" => "www.home.com",      //The requesting website be exactly the same as the website/URL associated with your PayTabs Merchant Account
           'return_url' => "http://localhost/expressv2/successredirect.php",
           "cms_with_version" => "API USING PHP",
             "paypage_info" => "1"
        );
        
        $dd=Paytab::create_pay_page($array);

        if($dd->response_code == '4012')
        {
            $array=array(
                'product_id'=>$dd->p_id,
                'amount'=>$amount
            );  
            return response()->json(['success' => true,'payments'=>$array], 200);
        }
        else
        {
            $array=[];
            return response()->json(['success' => false,'payments'=>$array], 200);   
        }
