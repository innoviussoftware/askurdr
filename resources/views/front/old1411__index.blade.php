...
@if($user->roles()->first()->id == '2')
	<div class="col-md-12 col-sm-12 col-xs-12 text-right">
  				...
  				<p>Welcome Dr.<?php echo Auth::user()->first_name ?> Your status is set to (Available)</p>
  				<?php
  				// dd(Auth::user()->id);
  				if(Auth::check()){
  				...
  				if($doctor_avoi->status){
  				 ?>
  				<a href="{{route('front.doctor.avoilabble')}}">
					<button type="button" class="btn btn-primary">Available</button>
				</a>
			<?php }else{ ?>
				<a href="{{route('front.doctor.notavoilabble')}}">
